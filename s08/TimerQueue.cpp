...
TimerQueue::~TimerQueue()
{
    ...
    // 即由weak_ptr尝试获得EventLoop的管理权，如果EventLoop不存在，那么Poller和EventLoop是组合关系，
...
{
    shared_ptr<Timer> timer {new Timer(cb, when, interval)};
    TimerId timerid(timer.get());
    assert(loop_.lock());
    // cout<<"lambda..."<<endl;

    loop_.lock()->runInLoop(std::bind(&TimerQueue::addTimerInLoop, this, timer));

    // cout<<"lambda done!"<<endl;
    return timerid;
}

void TimerQueue::addTimerInLoop(shared_ptr<Timer> timer){
    // cout<<"!!!"<<endl;
    loop_.lock()->assertInLoopThread();
    // cout<<"unique_ptr<Timer>: "<<bool(timer)<<endl;
    insert(move(timer));
}
...
vector<pair<Timestamp, shared_ptr<Timer>>> TimerQueue::getExpired(Timestamp now)
{
    vector<pair<Timestamp, shared_ptr<Timer>>> expired;
...
void TimerQueue::reset(std::vector<pair<Timestamp, shared_ptr<Timer>>>& expired, Timestamp when)
{
